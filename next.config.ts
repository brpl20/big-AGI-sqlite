...
const buildType =
  process.env.BIG_AGI_BUILD === 'standalone' ? ('standalone' as const) : process.env.BIG_AGI_BUILD === 'static' ? ('export' as const) : undefined;
...
let nextConfig: NextConfig = {
  ...
  // [sqlite] ensure SQLite packages are only used server-side
  serverExternalPackages: ['puppeteer-core', 'sqlite3', 'sqlite', 'bindings'],
  ...
  webpack: (config: any, { isServer }: { isServer: boolean }) => {
    ...
    // SQLite: exclude from client-side bundle (server-side only)
    if (!isServer) {
      ...
      // Ignore SQLite-related modules entirely on client-side
      const webpack = require('webpack');
      config.plugins = config.plugins || [];
      config.plugins.push(
        new webpack.IgnorePlugin({
          resourceRegExp: /^(sqlite3|sqlite|bindings)$/,
        }),
      );
    }
