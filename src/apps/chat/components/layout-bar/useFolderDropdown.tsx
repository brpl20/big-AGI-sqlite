...
export function useFolderDropdown(conversationId: DConversationId | null) {
  ...
  const handleFolderChange = React.useCallback(
    (folderId: string | null) => {
      if (conversationId && folderId) {
        ...
        if (folderId !== SPECIAL_ID_CLEAR_FOLDER) useFolderStore.getState().addConversationToFolder(folderId, conversationId);
      }
    },
  ...
  const folderDropdown = React.useMemo(() => {
    ...
    if (!enableFolders || !folderItems) return null;

    return <OptimaBarDropdownMemo items={folderItems} value={currentFolderId} onChange={handleFolderChange} placeholder="Assign to folder" showSymbols />;
