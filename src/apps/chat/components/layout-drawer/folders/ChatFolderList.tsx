...
import { DFolder, useFolderStore } from '~/common/stores/folders/store-chat-folders-sqlite';
...
export function ChatFolderList(props: {
  ...
  const handleDragEnd = React.useCallback(
    (event: DragEndEvent) => {
      ...
      if (oldIndex !== -1 && newIndex !== -1) useFolderStore.getState().moveFolder(oldIndex, newIndex);
    },
    [folderIds],
  );
  ...
  return (
    <Box sx={props.sx}>
      <List variant="plain" sx={listSx}>
        ...
        {/* Sortable folders */}
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd} modifiers={_dndModifiers}>
          <SortableContext items={folderIds} strategy={verticalListSortingStrategy}>
            {/* Folder Items */}
            {folders.map((folder) => (
              <FolderListItem key={folder.id} folder={folder} isActive={folder.id === activeFolderId} onFolderSelect={onFolderSelect} />
            ))}
          </SortableContext>
        </DndContext>
