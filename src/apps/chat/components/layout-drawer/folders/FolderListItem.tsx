...
import { DFolder, FOLDERS_COLOR_PALETTE, useFolderStore } from '~/common/stores/folders/store-chat-folders-sqlite';
...
export function FolderListItem(props: { folder: DFolder; isActive: boolean; onFolderSelect: (folderId: string | null) => void }) {
  // props
  ...
  // DnD Kit sortable
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: folderId });

  // handlers

  const handleFolderActivate = React.useCallback(
  ...
  const handleSetTitle = React.useCallback((newTitle: string, folderId: string) => {
    if (newTitle.trim()) useFolderStore.getState().setFolderName(folderId, newTitle.trim());
  ...
  const handleDeleteCanceled = React.useCallback(
    (event: React.MouseEvent) => {
      if (!deleteArmed) return;
      setDeleteArmed(false);
      event.stopPropagation();
  ...
  return (
    ...
    >
      <ListItemButton {...attributes} {...listeners} selected={isActive} onClick={handleFolderActivate} sx={_styles.itemButton}>
        <ListItemDecorator>
        ...
        {isEditing ? (
          ...
          <ListItemContent onDoubleClick={handleEditTitle}>{folderTitle}</ListItemContent>
        )}
