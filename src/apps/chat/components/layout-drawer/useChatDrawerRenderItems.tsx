...
import type { DFolder } from '~/common/stores/folders/store-chat-folders-sqlite';
import {
  DMessage,
  DMessageUserFlag,
...
function messageHasImageFragments(message: DMessage): boolean {
  return message.fragments.some(
    (fragment) => isContentOrAttachmentFragment(fragment) && isImageRefPart(fragment.part) /*&& fragment.part.dataRef.reftype === 'dblob'*/,
  );
}
...
function findOpenInViewIndices(chatPanesConversationIds: DConversationId[], ourId: DConversationId): string | false {
  ...
  return (
    chatPanesConversationIds
      .reduce((acc: string[], id, idx) => {
        if (id === ourId) acc.push((idx + 1).toString());
        return acc;
      }, [])
      .join(', ') || false
  );
...
export function useChatDrawerRenderItems(
  ...
  return useChatStore(({ conversations: convPreFilter }) => {
    ...
    const conversations = filterIsArchived ? convPreFilter.filter((c) => !!c.isArchived) : convPreFilter.filter((c) => !c.isArchived);

    // filter 1: select all conversations or just the ones in the active folder
    const conversationsInFolder = !activeFolder ? conversations : conversations.filter((_c) => activeFolder.conversationIds.includes(_c.id));
    ...
    const chatNavItems = conversationsInFolder
      .map((_c): ChatNavigationItemData | null => {
        ...
        if ((filterHasStars && !hasStars) || (filterHasImageAssets && !hasImages) || (filterHasDocFragments && !hasDocs)) return null;
        ...
        const userFlagsUnique = !messageFlags.size ? undefined : Array.from(messageFlags).map(messageUserFlagToEmoji).join('');
    ...
    if (isSearching && searchSorting === 'frequency') chatNavItems.sort((a, b) => b.searchFrequency - a.searchFrequency);
    ...
    // [grouping] group by date or persona
      ...
      const grouped = chatNavItems.reduce(
        (acc, item) => {
          ...
          switch (grouping) {
            ...
            case 'persona':
              ...
              break;
            case 'dimension':
              if (item.messageCount > 20) bucket = 'Large chats';
              else if (item.messageCount > 10) bucket = 'Medium chats';
              else if (item.messageCount > 5) bucket = 'Small chats';
              else if (item.messageCount > 1) bucket = 'Tiny chats';
              else if (item.messageCount === 1) bucket = 'Single message';
              else bucket = 'Empty chats';
              break;
          }
      ...
      renderNavItems = Object.entries(grouped).flatMap(([groupName, items]) => [{ type: 'nav-item-group', title: groupName }, ...items]);
    } else {
    ...
    const filteredChatsBarBasis =
      !isSearching && (!showRelativeSize || filteredChatsCount < 2)
        ? 0
