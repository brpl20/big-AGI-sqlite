...
import { useFolderStore } from '~/common/stores/folders/store-chat-folders-sqlite';
...
export function AppChat() {
  ...
  const beamOpenStoreInFocusedPane = focusedPaneIndex === null ? null : !beamsOpens?.[focusedPaneIndex] ? null : (paneBeamStores?.[focusedPaneIndex] ?? null);
  ...
  const handleMessageRegenerateLastInFocusedPane = React.useCallback(async () => {
    ...
    if (lastMessage.role === 'assistant') cHandler.historyTruncateTo(lastMessage.id, -1);
  ...
  const handleMessageBeamLastInFocusedPane = React.useCallback(async () => {
    ...
    const lastMessage = inputHistory[inputHistory.length - 1];
    if (lastMessage.role === 'assistant') cHandler.beamInvoke(inputHistory.slice(0, -1), [lastMessage], lastMessage.id);
    else if (lastMessage.role === 'user') cHandler.beamInvoke(inputHistory, [], null);
  ...
  const handleConversationNewInFocusedPane = React.useCallback(
    (forceNoRecycle: boolean, isIncognito: boolean) => {
      // create conversation (or recycle the existing top-of-stack empty conversation)
      const conversationId =
        recycleNewConversationId && !forceNoRecycle && !isIncognito
          ? recycleNewConversationId
      ...
      // focus the composer
      if (!isMobile) composerTextAreaRef.current?.focus();
    },
  ...
  const focusedChatPanelContent = React.useMemo(
    () =>
      !focusedPaneConversationId ? null : (
        <ChatPane
  ...
  React.useEffect(() => {
    ...
    if (Release.IsNodeDevBuild && intent.initialConversationId === 'null') openConversationInFocusedPane(null! /* for debugging purporse */);
    // Open the initial conversation if set
    else if (intent.initialConversationId) openConversationInFocusedPane(intent.initialConversationId);
    // Create a new chat if requested
    else if (intent.newChat !== undefined) handleConversationNewInFocusedPane(false, false);
  ...
  const handleMoveFocus = React.useCallback((direction: number, wholeList?: boolean) => {
    ...
    if (activeElement) {
      ...
      if (!messageListElement) messageListElement = activeElement.closest('[role=chat-messages-list]') as HTMLElement;
      else withinBeam = true;
    } else messageListElement = document.querySelector('[role=chat-messages-list]') as HTMLElement;
    ...
    const nextIndex =
      wholeList && direction < 0
        ? 0
        : snapToBottom
          ? messageElements.length - 1
          : isAtBottom && direction < 0
            ? currentIndex
            : currentIndex === -1
              ? direction < 0
                ? 0
    ...
    <>
              ...
              <ErrorBoundary>
                <Panel
                  ...
                  onClick={(event) => {
                    ...
                    if (event.altKey && chatPanes.length > 1) return setFocusedPaneIndex(-1);
                ...
                >
                  ...
                  <ScrollToBottom bootToBottom stickToBottomInitial disableAutoStick={isMobile && _paneBeamIsOpen} sx={scrollToBottomSx}>
                    {!_paneBeamIsOpen && (
                      <ChatMessageList
                        conversationId={_paneConversationId}
      ...
      {!!diagramConfig && (
        <React.Suspense fallback={null}>
          <DiagramsModalLazy config={diagramConfig} onClose={() => setDiagramConfig(null)} />
        </React.Suspense>
      )}
      ...
      {!!tradeConfig && (
        <React.Suspense fallback={null}>
          <TradeModalLazy config={tradeConfig} onConversationActivate={handleOpenConversationInFocusedPane} onClose={() => setTradeConfig(null)} />
        </React.Suspense>
      )}
    </>
  );
