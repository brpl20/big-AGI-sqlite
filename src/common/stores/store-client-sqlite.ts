...
const useDeviceStore = create<PerClientStore>()(
  sqlitePersist(
    () => ({
      // initial state
      localDeviceId: '',
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          console.log('[SQLite] Device store rehydrated:', state);
        }
      },
      migrate: (persistedState: any, version: number) => {
        console.log(`[SQLite] Migrating device store from version ${version}`);
...
// Export the store for external use (debugging/testing)
export { useDeviceStore };

// Utility functions for manual testing of SQLite operations
export async function testSqliteDeviceStore() {
  console.log('[SQLite Test] Testing device store operations...');

  const currentState = useDeviceStore.getState();
  console.log('[SQLite Test] Current state:', currentState);

  // Test manual save
  try {
    await currentState.persist.clearStorage();
    console.log('[SQLite Test] Storage cleared successfully');

    // Force rehydration
    await currentState.rehydrate();
    console.log('[SQLite Test] Rehydration completed');

    const newState = useDeviceStore.getState();
    console.log('[SQLite Test] State after rehydration:', newState);
  } catch (error) {
    console.error('[SQLite Test] Error during test:', error);
  }
}
