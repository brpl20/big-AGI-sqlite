...
export const useClientWorkspaceStore = create<WorkspaceState & WorkspaceActions>()(
  sqlitePersist(
    (_set, _get) => ({
      ...
      liveFileAssign: (workspaceId: DWorkspaceId, fileId: LiveFileId) =>
        _set((state) => {
          // if already included, do not do anything
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        // [GC][LiveFile] remove LiveFile references to invalid objects (also done in chats.converters.ts)
