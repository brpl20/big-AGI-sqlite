...
export namespace V4ToHeadConverters {
  export function inMemHeadCleanDConversations(cs: ReadonlyArray<DConversation>): void {
    ...
    for (const c of cs) _inMemHeadCleanDConversation(c, validLiveFileIDs);
  }
  ...
  function _inMemHeadCleanDConversation(c: DConversation, validLiveFileIDs: LiveFileId[]): void {
    ...
    for (const message of c.messages) inMemHeadCleanDMessage(message, validLiveFileIDs);
  }
  ...
  export function inMemHeadCleanDMessage(m: DMessage, validLiveFileIDs: LiveFileId[]): void {
    ...
    for (let i = 0; i < m.fragments.length; i++) {
      ...
      if (isAttachmentFragment(fragment) && fragment.liveFileId) if (!validLiveFileIDs.includes(fragment.liveFileId)) delete fragment.liveFileId;

      // show the aborted ops: convert a Placeholder fragment [part.pt='ph'] to an Error fragment
      if (
        (isVoidFragment(fragment) && isPlaceholderPart(fragment.part)) ||
...
export namespace V3StoreDataToHead {
  ...
  export function recreateConversation(
    ic: ImportConversationV3 | DataAtRestV1.RestChatJsonV1 /* this is here because a chat at rest JsonV1 basically overlapped with V3 data */,
    // | DConversation /* this is here because a V4 could have been exported too, and we want to force type overlaps */
  ): DConversation {
    const { id, userTitle, autoTitle, systemPurposeId, messages, updated, created } = ic;
  ...
  function _recreateMessage(im: ImportMessageV3 | DMessage): DMessage {
    ...
    if (!cm) {
      const { id, text, role, purposeId, originLLM, metadata, userFlags, tokenCount, created, updated } = im as ImportMessageV3;
  ...
  export type ImportMessageV3 = {
    ...
    metadata?: {
      // metadata, mainly at creation and for UI
...
export namespace DataAtRestV1 {
  ...
  export function formatAllToJsonV1B(
    conversations: DConversation[],
    modelSources: DModelsService[],
    folders: DFolder[],
