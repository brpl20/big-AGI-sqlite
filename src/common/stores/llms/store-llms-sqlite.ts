...
import { llmSqlitePersist } from '../../../lib/db/zustand-llm-sqlite-middleware';
...
import {
  createLlmsAssignmentsSlice,
  LlmsAssignmentsActions,
  LlmsAssignmentsSlice,
  LlmsAssignmentsState,
  llmsHeuristicUpdateAssignments,
} from './store-llms-domains_slice';
import { getDomainModelConfiguration } from './hooks/useModelDomain';
import { portModelPricingV2toV3 } from './llms.pricing';

/// ModelsStore - a store for configured LLMs and configured services with SQLite persistence
      ...
      (set, get, _store) => ({
        ...
        setServiceLLMs: (serviceId: DModelsServiceId, serviceLLMs: ReadonlyArray<DLLM>, keepUserEdits: boolean, keepMissingLLMs: boolean) =>
          set(({ llms: existingLLMs, modelAssignments }) => {
            ...
            if (!keepMissingLLMs) existingLLMs = existingLLMs.filter((llm) => llm.sId !== serviceId);
        ...
        rerankLLMsByServices: (serviceIdOrder: DModelsServiceId[]) =>
          set((state) => {
            ...
            const serviceIdToIndex = serviceIdOrder.reduce(
              (acc, sId, idx) => {
                acc[sId] = idx;
                return acc;
              },
        ...
        updateLLMUserParameters: (id: DLLMId, partialUserParameters: Partial<DModelParameterValues>) =>
          set(({ llms }) => ({
            llms: llms.map((llm: DLLM): DLLM => (llm.id === id ? { ...llm, userParameters: { ...llm.userParameters, ...partialUserParameters } } : llm)),
          })),
        ...
        createModelsService: (vendor: IModelVendor): DModelsService => {
          ...
          function _relabelServicesFromSameVendor(vendorId: ModelVendorId, services: DModelsService[]): DModelsService[] {
            let n = 0;
            return services.map(
              (s: DModelsService): DModelsService => (s.vId !== vendorId ? s : { ...s, label: s.label.replace(/ #\d+$/, '') + (++n > 1 ? ` #${n}` : '') }),
            );
          }
        ...
        updateServiceSettings: <TServiceSettings>(id: DModelsServiceId, partialSettings: Partial<TServiceSettings>) =>
          set((state) => ({
            sources: state.sources.map((s: DModelsService): DModelsService => (s.id === id ? { ...s, setup: { ...s.setup, ...partialSettings } } : s)),
          })),

        setConfServiceId: (id: DModelsServiceId | null) => set({ confServiceId: id }),

        setOpenRouterKey: (key: string) =>
          set((state) => {
            const firstOpenRouterService = state.sources.find((s) => s.vId === 'openrouter');
            return !firstOpenRouterService
              ? state
              : {
                  sources: state.sources.map(
                    (s: DModelsService): DModelsService =>
                      s.id === firstOpenRouterService.id ? { ...s, setup: { ...s.setup, oaiKey: key satisfies DOpenRouterServiceSettings['oaiKey'] } } : s,
                  ),
                };
          }),
      }),
      {
        name: 'app-models-sqlite',
        version: 4,
        migrate: (_state: any, fromVersion: number): LlmsStore => {
          ...
          if (fromVersion < 1)
            for (const llm of state.llms)
              if (llm.maxOutputTokens === undefined) llm.maxOutputTokens = llm.contextTokens ? Math.round(llm.contextTokens / 2) : null;
        ...
        onRehydrateStorage: () => (state) => {
          ...
          console.log('[LLM Store SQLite] Post-loading validation and migrations...');

          try {
            // [GC] remove models that do not refer to a valid service
            state.llms = state.llms
              .map((llm: DLLM): DLLM | null => {
            ...
            if (!state.modelAssignments || !Object.keys(state.modelAssignments).length) {
              console.log('[LLM Store SQLite] Auto-selecting initial model assignments...');
              state.modelAssignments = llmsHeuristicUpdateAssignments(state.llms, {});
            }

            // Validate and migrate legacy pricing data
            try {
              state.llms.forEach(portModelPricingV2toV3);
            } catch (error) {
              console.warn('[LLM Store SQLite] Error migrating pricing data:', error);
            }

            // Migrate legacy options to parameters format
            try {
              state.llms.forEach(_port_V3Options_to_V4Parameters_inline);
            } catch (error) {
              console.warn('[LLM Store SQLite] Error migrating parameters:', error);
            }

            console.log(
              `[LLM Store SQLite] Loaded ${state.sources.length} services, ${state.llms.length} models, ${Object.keys(state.modelAssignments).length} assignments`,
            );
          } catch (error) {
            console.error('[LLM Store SQLite] Error in post-loading validation:', error);
          }
        },
      },
    ),
...
export function findLLMOrThrow(llmId: DLLMId): DLLM {
  ...
  if (!llm) throw new Error(`Large Language Model ${llmId} not found`);
  return llm;
}
...
export function getDomainModelIdOrThrow(
  tryDomains: DModelDomainId[],
  requireFunctionCallTools: boolean,
  ...
  throw new Error(
    `No model available for '${useCaseLabel}'. Pease select a '${tryDomains[0]}' model that supports${requireFunctionCallTools ? ' function calls' : ' text input'}${requireImageInput ? ' and image input' : ''} in App Preferences > Chat AI.`,
  );
}
...
function _port_V3Options_to_V4Parameters_inline(llm: DLLM): void {
  ...
  if ('options' in llmV3 && typeof llmV3.options === 'object') {
    if ('llmRef' in llmV3.options) llm.initialParameters.llmRef = llmV3.options.llmRef;
...
// Utility functions for manual testing of SQLite operations
export async function testSqliteLLMStore() {
  console.log('[SQLite Test] Testing LLM store operations...');

  const currentState = useModelsStore.getState();
  console.log('[SQLite Test] Current LLM state:', {
    services: currentState.sources.length,
    models: currentState.llms.length,
    assignments: Object.keys(currentState.modelAssignments).length,
  });

  // Test manual save
  try {
    await currentState.persist.clearStorage();
    console.log('[SQLite Test] LLM storage cleared successfully');

    // Force rehydration
    await currentState.rehydrate();
    console.log('[SQLite Test] LLM rehydration completed');

    const newState = useModelsStore.getState();
    console.log('[SQLite Test] LLM state after rehydration:', {
      services: newState.sources.length,
      models: newState.llms.length,
      assignments: Object.keys(newState.modelAssignments).length,
    });
  } catch (error) {
    console.error('[SQLite Test] Error during LLM test:', error);
  }
}
