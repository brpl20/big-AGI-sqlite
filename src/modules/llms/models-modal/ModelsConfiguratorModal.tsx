...
// configuration
const MODELS_WIZARD_ENABLE_INITIALLY = false;
...
export function ModelsConfiguratorModal(props: {
  ...
  const activeServiceId = confServiceId ?? modelsServices[modelsServices.length - 1]?.id ?? null;
  ...
  React.useEffect(() => {
    if (triggerWizard) setTab('wizard');
  }, [triggerWizard]);
  ...
  const startButton = React.useMemo(() => {
    ...
    if (!hasAnyServices)
      return (
        <Button variant="outlined" color="neutral" onClick={handleShowWizard} sx={{ backgroundColor: 'background.popup' }}>
    ...
    >
      ...
      {isTabSetup && (
        <Box sx={{ display: 'grid', gap: 'var(--Card-padding)' }}>
          {activeService ? <LLMVendorSetup service={activeService} /> : <Box sx={{ minHeight: '7.375rem' }} />}
        </Box>
      )}
